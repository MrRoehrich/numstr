...
\end{equation}
wobei $\Phi$ einen passiven Skalar und $\Delta V$ ein Volumen darstellen, angenommen werden. Ein Beispiel hierfür ist die Dichte $\rho$:
\begin{equation}
...
\end{equation}
Die linke Seite dieser Gleichung spiegelt die substantielle Beschleunigung, welche sich aus lokaler und konvektiver Beschleunigung zusammensetzt, wieder. Die rechte Seite setzt sich aus der Summe von Druck-, Volumen- (in diesem Fall Gewichts-) und Reibungskräften zusammen. \newpage
...
\end{equation}
Für die Stromfunktion $\Psi$ ist es genau der andere Fall. Das Vektorpotential $\vec{\Psi}$ erfüllt wegen $div\left(rot \ \vec{\Psi} \right)=0$ bereits \eqref{eq:konti_incompressibel}, somit folgt die Bestimmungslgleichung der dritten Komponente des Potentials hier aus der Forderung nach Rotationsfreiheit der Strömung:
...
\begin{equation}
c_{p}=\dfrac{\Delta p}{\frac{\rho}{2}\cdot |\vec{v}_{ref}|^{2}} = \dfrac{p-p_{ref}}{\frac{\rho}{2}\cdot |\vec{v}_{ref}|^{2}}=1-\dfrac{|\vec{v}|^{2}}{|\vec{v}_{ref}|^{2}}.
\end{equation}
Als Referenzgeschwindigkeit in $x$-Richtung kann beispielsweise die Einströmgeschwindigkeit $u_{\infty}$ des Fluid in den Kanal verwendet werden.
...
Die für dieses Problem verwendete Diskretisierung geschieht mittels Finiter Differenzen. 
...
\begin{align}
\xi\left(x,y\right)&=\left(I-1\right)\cdot\dfrac{x-x_{min}}{x_{max}-x_{min}}, &&\eta\left(x,y\right)=\left(J-1\right)\cdot\dfrac{y-h_{u}(x)}{h_{o}(x)-h_{u}(x)}, \label{trafo_xi_eta} \\
x\left(\xi,\eta\right)&=x_{min}+\dfrac{x_{max}-x_{min}}{I-1}\cdot\xi, &&y\left(\xi,\eta\right)=h_{u}(x)+\dfrac{h_{o}(x)-h_{u}(x)}{J-1}\cdot\eta,\label{trafo_x_y}
\end{align}
...
Im Folgenden folgt eine Konsistenzuntersuchung für die in Abschnitt \ref{seq:diskretisierung} diskretisierte Laplace-Gleichung (vgl. \ref{eq:fd_laplace_phi}). Mittels Talyorentwicklung folgt:
\begin{align}
...
\end{equation}
Somit gibt die Differenzenlösung eine hinreichend genaue Lösung für Gleichung \eqref{eq:laplace_potentialfkt}. Das Gleiches folgt für die Stromfunktion $\Psi$.
...
\item Auf einem $50\times50$-Gitter werden als Anfangsdaten bereits die Werte der analytischen Lösung vorgegeben. Da die Methode nichts berechnen muss, sollte keine Iteration ausgeführt werden.
...
\\[0.2cm] Aufgrund der Rotationssymmetrie des Laplace-Operators (siehe \eqref{eq:laplace_potentialfkt} und \eqref{eq:laplace_stromfkt}), ist dieses Ergebnis auf die verbleibenden drei Quadranten entsprechend zu spiegeln.
\end{enumerate}
Es ist erkennbar, dass alle getesteten Potentialströmungen hinreichend exakte Ergebnisse liefern (die Ergebnisse sind nicht exakt der analytischen Lösung aufgrund Rundungsfehler infolge der Maschinengenauigkeit).
