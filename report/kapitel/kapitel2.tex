...
\end{align}
wobei $f_{\bullet,\bullet}=\rho\cdot u_{\bullet,\bullet}$ und $g_{\bullet,\bullet}=\rho\cdot v_{\bullet,\bullet}$. Für die Peclet-Zahl gilt $Pe_{\bullet,\bullet}=\dfrac{\rho u_{\bullet,\bullet} l}{\alpha}$, mit charakteristischer Länge $l$, die hier zu Eins gesetzt worde ist, und es gilt zum Beispiel $D_{i,j+1}=\dfrac{\alpha}{\Delta x}$. Für die Berechnung von $A\left(|Pe|\right)$ können die folgenden verschiedenen Schemata verwendet werden:
\FloatBarrier
\begin{table}[!htb]
\centering
  \begin{tabular}{ c | c }
...
\end{table}
\FloatBarrier
Das Strömungsfeld wird vorgegeben, zunächst sei $u=1$\textcolor{red}{EINHEITEN SCHREIBEN??!!} und $v=0$. Es wird die Upwind-Methode verwendet, d.h. als stationärer Endwert in der mittleren Zelle muss sich der Wert der linken Zelle, sprich der Wert $1$, durchsetzen. Die Resultate der numerischen Berechnung geben genau dieses Ergebnis wieder:
\FloatBarrier
...
\end{minipage}
\FloatBarrier
Nun wird das Geschwindigkeitsfeld zu $u=0$ und $v=1$ geändert\textcolor{red}{EINHEITEN SCHREIBEN??!!}. Aufgrund der gleichen Argumentation, muss sich jetzt der Wert der unteren Zelle, sprich der Wert $0$ in der mittleren Zelle durchsetzen. Die numerische Simulation stimmt mit dieser Überlegung überein:
\FloatBarrier
\begin{minipage}[t]{7.5cm}
\includegraphics[height=5.5cm]{bilder/kapitel2/konvektion_1}
\captionof{figure}{Upwind-Methode, Initialzustand}
...
\item Die Software gibt anscheinend für das Diffusionsproblem das richtige Ergebnis aus. Abschließend wird eine diffusive Strömung auf einem größeren Gitter der Größe $100\times100$ mit Diffusionskoeffizient $\alpha=50$\textcolor{red}{EINHEITEN SCHREIBEN??!!} simuliert. Das Integrationsgebiet ist ebenfalls $100\times100$\textcolor{red}{EINHEITEN SCHREIBEN??!!} groß. Im Folgenden wird die zeitliche Evolution des skalaren Wertes $\Phi$ über das gesamte Integrationsgebiet dargestellt:
\begin{figure}[!htb]
\begin{minipage}[t]{3.9cm}
...
\begin{enumerate}
\item Im Folgenden wird eine Konvektion-Diffusions-Strömung visualisiert. Die Parameter sind $\alpha=50$, $u=5$ und $v=10$. Das Gebiet ist das selbe $100\times100$-Gebiet, wie in den vorherigen Testfällen, wo Konvektion und Diffusion getrennt betrachtet wurden. Das Ergebnis der Simulation sieht wie folgt aus:
\begin{figure}[!htb]
\begin{minipage}[t]{3.9cm}
...
\subsection{Quellen}
Es wird wieder eine $50\times50$-Diskretisierung für ein $50\times50$ großes Integrationsgebiet verwendet. Es werden undruchlässige Wände an allen vier Wänden simuliert, die Konvektionsgeschwindigkeiten sind beide Null und der Diffusionskoeffizient beträgt $\alpha=10$. Als Initialwerte für den Skalar $\Phi$ wird wieder über dem gesamten Gebiet der Wert $4,711$ gesetzt. An der Stelle $(1,1)$ befindet eine Quelle, die pro Sekunde $10$ Einheiten des Skalars $\Phi$ in das Integrationsgebiet einbringt, an der Stelle $(2,2)$ befindet sich eine Senke die $10$ Einheiten des Skalars $\Phi$ pro Sekunde "absaugt". Es ergibt sich folgende, physikalisch korrektes, zeitliche Evolution.
% 50x50 Diskretisierung, 3x3 physikalisches Gebiet, Zelle 816 s=+10, zelle 1683 s=-10
\begin{figure}[!htb]
\begin{minipage}[t]{3.9cm}
\includegraphics[width=3.8cm]{bilder/kapitel2/neumann_1_0sek}
\subcaption*{$t=0sek$}
...
\subcaption*{$t=10sek$}
\end{minipage}
\captionof{figure}{zeitliche Evolution diffusiver Strömung mit 2 Quellen}
\end{figure}
\FloatBarrier