...
double h = (data->yMax - data->yMin) * (1. - 1. / data->nCellsY);
   ...
   sCell* curCell;
   for (int cId = 0; cId < data->nCells; cId++) {      
      ybar = curCell->y - (data->yMax - data->yMin) / (2. * data->nCellsY);
      curCell = &data->cells[cId];
      ...
      // boundary-velocities:
      if ((cId + data->nCellsX) % data->nCellsX == 0) { // links
         ...
         curCell->faces[XM]->v = 0.;
      } else if ((cId - (data->nCellsX - 1)) % (data->nCellsX) == 0) { // rechts
         curCell->bTypeVelocity = DIRICHLET;
      ...
      if (cId < data->nCellsX) { // unten
         ...
         curCell->faces[XP]->u = 0.;
         curCell->faces[XP]->v = 0.;
      } else if (data->nCells - (cId + 1) < data->nCellsX) { // oben
         curCell->bTypeVelocity = DIRICHLET;
         curCell->faces[YP]->u = 0.;
         curCell->faces[YP]->v = 0.;
         curCell->faces[XM]->u = 0.;
...
void setDrivenCavityBoundaries(sData* data)
{
   ...
   for (int cId = 0; cId < data->nCells; cId++) {
      ...
      if (cId < data->nCellsX) { // unten
         ...
         // ACHTUNG!!: BEI DER AUSGABE WIRD DIE OBERE ZELLE GEMITTELT, DESWEGEN STEHT DORT u!=5
         curCell->faces[YP]->v = 0.;
         curCell->faces[XM]->u = 0.;
         curCell->faces[XM]->v = 0.;
         curCell->faces[XP]->u = 0.;
...
void setGartenschlauchBoundaries(sData* data) {
   ...
   for (int cId = 0; cId < data->nCells; cId++) {
      ...
      if ((cId + data->nCellsX) % data->nCellsX == 0) { // links
         curCell->faces[YM]-> u = U;
      } 
      
      if ((cId - (data->nCellsX - 1)) % (data->nCellsX) == 0) { // rechts
         curCell->p = pressure;
      }
